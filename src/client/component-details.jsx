...
const Stack = require('./component-stack.jsx');
...
module.exports = React.createClass({
  ...
  renderStackTrace: function() {
    if (this.props.type === 'message') {
      const sample = _.first(this.state.data);
  ...
  renderTable: function() {
    const isShowingBrowsersBreakdown = (
      this.props.type === 'message' &&
      !this.state.showingMeta
    );

    let items = _.map(this.state.data, function(data) {
      return <ReportItem
        key={ data.key }
  ...
  updateData: function() {
    const report = Reports.get(this.props.type, {
      id: this.props.id,
