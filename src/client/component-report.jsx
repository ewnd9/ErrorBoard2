...
const ReportItem = require('./component-report-item.jsx');
const Notice = require('./component-notice.jsx');
const UpdateCounter = require('./component-update-counter.jsx');
...
module.exports = React.createClass({
  ...
  render: function() {
    const latest = this.state.timespanLatest;
    const earliest = _.reduce(this.state.index, getEarliest, latest);

    const items = _.map(this.state.index, function(data) {
      return <ReportItem
        key={ data.key }
  ...
  empty: function() {
    const label = _.isNull(this.state.index) ? 'Loading…' : 'Nothing to display';
  ...
  createIndex: function() {
    ...
    const index = _.map(data, addKey).sort(sortByLatestReport);
  ...
  updateIndex: function() {
    ...
    const addIndex = _.partial(addCurrentIndex, this.state.index);
    const addDelta = _.partial(countDelta, this.state.index);
    const indexed = _.map(data, _.compose(addDelta, addIndex, addKey));

    const rows = partition(indexed, isIndexed);
...
function countDelta(index, item) {
  const prev = index[item._index] || {};
  const count = prev.count || 0;
  const delta = prev.delta || 0;
